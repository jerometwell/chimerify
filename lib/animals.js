...
function calculateHeadTop(body, head) {
    const top = body.y + head.y;
    if(top < 0) {
        return 0;
    }
...
function calculateHeadLeft(body, head) {
    const left = body.x + head.x;
    if(left < 0) {
        return 0;
    }
...
async function mergeAnimals({head: headAnimal, body: bodyAnimal}) {
    ...
    const image = bodyAnimal
            .bodyImage
            .composite( [ {input: transformedHead,  top: calculateHeadTop(head, body), left: calculateHeadLeft(head, body)}] )
